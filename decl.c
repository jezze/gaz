...
static int declarator(int arg, int scls, char *name, int *pprim, int *psize, int *pval, int *pinit);
...
int primtype(int t, char *s)
{
    ...
    p = t == CHAR ? PCHAR : t == INT ? PINT : t == STRUCT ? PSTRUCT : t == UNION ? PUNION : PVOID;
...
static int pmtrdecls(void)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            if (Token != CHAR && Token != INT && Token != VOID && Token != STRUCT && Token != UNION)
            {
...
int pointerto(int prim)
{
    ...
    if (CHARPP == prim || INTPP == prim || VOIDPP == prim || FUNPTR == prim || (prim & STCMASK) == STCPP || (prim & STCMASK) == UNIPP)
    ...
    return PINT == prim ? INTPTR : PCHAR == prim ? CHARPTR : PVOID == prim ? VOIDPTR : INTPTR == prim ? INTPP : CHARPTR == prim ? CHARPP : VOIDPP;
...
static int declarator(int pmtr, int scls, char *name, int *pprim, int *psize, int *pval, int *pinit)
{
    ...
    else if (LPAREN == Token)
    {
        ...
        if (*pprim != PINT)
            error("function pointers are limited to type 'int'", NULL);
                ...
                *psize = 1;
                ...
                if (ASSIGN == Token)
                {
                    ...
                    *psize = initlist(name, *pprim);
                    ...
                    if (CAUTO == scls)
                        error("initialization of local arrays not supported: %s", name);
                ...
                else if (CEXTERN != scls)
                {
                    ...
                    error("automatically-sized array lacking initialization: %s", name);
...
static int localdecls(void)
{
    ...
    while (AUTO == Token || EXTERN == Token || REGISTER == Token || STATIC == Token || INT == Token || CHAR == Token || VOID == Token || ENUM == Token || STRUCT == Token || UNION == Token)
    {
        ...
        if (AUTO == Token || REGISTER == Token || STATIC == Token || EXTERN == Token)
        {
...
static void structdecl(int clss, int uniondecl)
{
    ...
    y = addglob(sname, uniondecl? PUNION: PSTRUCT, TSTRUCT, CMEMBER, 0, 0, NULL, 0);
    Token = scan();

    while (INT == Token || CHAR == Token || VOID == Token || STRUCT == Token || UNION == Token)
    {
        ...
        for (;;)
        {
            ...
            prim = base;
            type = declarator(1, clss, name, &prim, &size, &dummy, &dummy);

            addglob(name, prim, type, CMEMBER, size, addr, NULL, 0);

            size = objsize(prim, type, size);

            if (0 == size)
                error("size of struct/union member is unknown: %s", name);
